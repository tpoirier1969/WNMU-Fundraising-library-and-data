(() => {
  ...
  async function unarchiveProgram(reference = {}, payload = {}) {
    ...
    if (!directId && !programId) {
      return { data: null, error: new Error('Could not find an exact program row ID to clear archive state.') };
    }

    const attempts = [];
    if (directId) attempts.push(['id', directId]);
    if (programId && programId !== directId) attempts.push(['program_id', programId]);

    let lastResponse = { data: null, error: new Error('Could not find an exact program row to clear archive state.') };
    for (const [field, value] of attempts) {
      let query = state.client.from(constants.BASE_TABLE).update(cleanedPayload).eq(field, value);
      const response = await query.select('id, program_id, nola_code, title, rights_end, status, library_state, is_archived, archived, inactive_flag').maybeSingle();
      lastResponse = response;
      if (response.error) {
        const message = String(response.error.message || '').toLowerCase();
        if (field === 'program_id' && message.includes('column') && message.includes('program_id')) continue;
        return response;
      }
  ...
  function aggregateImportedSnapshot(entries = []) {
    ...
    for (const entry of entries) {
      const row = entry.row || {};
      const key = utils.normalizeText(row.row_hash || row.id || `index:${entry.index}`);
    ...
    for (const field of preferredFields) {
      const value = unique.map((entry) => entry.row?.[field]).find((candidate) => utils.normalizeText(candidate || ''));
      if (value != null && utils.normalizeText(value || '')) merged[field] = value;
    }
  ...
  function canonicalizeImportedAirings(rows = []) {
    ...
    (Array.isArray(rows) ? rows : []).forEach((row, index) => {
      ...
      const naturalKey = identity && dateKey && timeKey ? importNaturalKey(row) : '';
      if (!naturalKey) {
