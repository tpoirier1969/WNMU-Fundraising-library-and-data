(() => {
  ...
  function bindEvents() {
    ...
    els.libraryBody?.addEventListener('click', async (event) => {
      ...
      const explicitlyArchived = App.derive.hasExplicitArchiveState(row || {});
      const payload = {};
      if (Object.prototype.hasOwnProperty.call(row || {}, 'is_archived')) payload.is_archived = false;
      if (Object.prototype.hasOwnProperty.call(row || {}, 'archived')) payload.archived = false;
      if (Object.prototype.hasOwnProperty.call(row || {}, 'inactive_flag')) payload.inactive_flag = false;
      if (Object.prototype.hasOwnProperty.call(row || {}, 'status')) payload.status = 'active';
      if (Object.prototype.hasOwnProperty.call(row || {}, 'library_state')) payload.library_state = 'active';

      if (!explicitlyArchived) {
        setNotice('This title is not explicitly archived. It is hidden because the rights-end date has expired, so extend the rights date instead.', 'warn');
        return;
      }
      if (!Object.keys(payload).length) {
        setNotice('I found the row, but there are no explicit archive fields on it to clear.', 'warn');
        return;
      }
      if (!(dbId || row?.id || programDbId || row?.program_id)) {
        setNotice('Could not find an exact database row ID for this archived title.', 'warn');
        return;
      }
      ...
      try {
        ...
        if (!response?.data?.id) {
          setNotice('No exact archived row was updated. I stopped instead of pretending that worked.', 'warn');
          return;
        }
        ...
        if (rightsStillExpired) {
          ...
          (state.rawRows || []).forEach(applyLocalRestore);
          App.listUi.applyLibraryView();
          setNotice('Archive flag cleared. This title still has a past rights-end date, so it was moved into Active only temporarily so you can edit and extend the date.');
        } else {
          await refreshAll({ preserveDetail: true, workspace: state.activeWorkspace });
