(() => {
  ...
  function renderImpact(s) {
    ...
    $('#premium-impact').innerHTML = `
      ...
      <div class="premium-impact-callout">
        <strong>Association is not lift.</strong>
        Premium-taking donors averaged <strong>${money(association.averagePremiumPledge)}</strong> versus <strong>${money(association.averageNoPremiumPledge)}</strong> for donors who took no premium
        (${associationDelta >= 0 ? '+' : ''}${money(associationDelta)}${associationDeltaPct == null ? '' : `, ${associationDeltaPct >= 0 ? '+' : ''}${pct(associationDeltaPct)}`}).
        That difference does <strong>not</strong> tell us what those same donors would have given without premiums.
      </div>
      <div class="premium-impact-evidence-grid">
      ...
      <p class="premium-note">Across comparable fundraisers, premium-taking donors had the higher average pledge in <strong>${num(association.fundraisersPremiumAverageHigher || 0)}</strong> drive(s) and the lower average in <strong>${num(association.fundraisersPremiumAverageLower || 0)}</strong>. This is useful context, but donor self-selection means neither group is a control group.</p>
      <h3>Best historical comparisons available</h3>
      ${comparisons.length ? table(
...
,'Mapping evidence'],
        comparisons.slice(0,12).map((item)=>`<tr>
        ...
        { minWidth: 1050 }
      ) : '<div class="premium-empty">No same-title / different-package comparisons are mapped yet. More historical premium mappings will make this section more useful.</div>'}
      <div class="premium-impact-causal-note"><strong>Causal estimate: Not established.</strong> ${esc(causal.reason || '')}</div>`;
  }
