(() => {
  ...
  function selectionSnapshot() {
    ...
    return {
      ...
      categories:A.categoryAnalysis(mappedRows), packages:A.packageAnalysis(mappedRows),
  ...
  function render() {
    ...
    $('#premium-report-output').innerHTML=`<article class="one-sheet premium-report-sheet">
      ...
      <section><div class="section-heading"><h2>Exact premium packages</h2><p>Bundle/package performance without splitting a donor selection into artificial component revenue.</p></div>${table(['Fundraiser','Premium / package','Categories','Pledges','Pledged','Avg pledge','Cost','Cost %','Est. net'],s.packages.slice(0,40).map((r)=>`<tr><td>${esc(r.fundraiserLabel)}</td><td><strong>${esc(r.description)}</strong></td><td>${esc((r.componentCategories||[]).join(', '))}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${money(r.totalPremiumCost,2)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Category trends</h2><p>A package can carry multiple component tags, so these rows are comparative and must not be summed together.</p></div>${table(['Category tag','Packages','Fundraisers','Pledges','Pledged','Avg pledge','Cost %'],s.categories.map((r)=>`<tr><td><strong>${esc(r.category)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${pct(r.costPercentOfPledged)}</td></tr>`))}</section>
