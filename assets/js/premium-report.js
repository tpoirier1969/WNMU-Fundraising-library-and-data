(() => {
  ...
  function selectionSnapshot() {
    ...
    return {
      ...
      categories:A.categoryAnalysis(mappedRows), compositions:A.compositionAnalysis(mappedRows), brandScopes:A.brandScopeAnalysis(mappedRows), packages:A.packageAnalysis(mappedRows),
  ...
  function render() {
    ...
    $('#premium-report-output').innerHTML=`<article class="one-sheet premium-report-sheet">
      ...
      <section><div class="section-heading"><h2>Fundraiser economics</h2><p>Premium-taking behavior and cost by drive.</p></div>${table(['Fundraiser','Pledged','Pledges','Premium take rate','Premium ,s.fundraiserAnalysis.map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${money(r.totalPledged)}</td><td>${num(r.totalPledges)}</td><td>${pct(r.premiumTakeRate)}</td><td>${money(r.anyPremiumDollars)}</td><td>${money(r.totalPremiumCost)}</td><td>${pct(r.premiumCostPercentOfTotal)}</td><td>${money(r.estimatedTotalNetAfterPremium)}</td><td>${money(r.averagePremiumPledge)}</td><td>${money(r.averageNoPremiumPledge)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Exact premium packages</h2><p>Each selectable premium remains its own accounting record. The normalized composition lets us compare similar package recipes across different titles.</p></div>${table(['Fundraiser','Premium / package','Package composition','Brand / source','Pledges','Pledged','Avg pledge','Cost','Cost %','Net after premium'],s.packages.slice(0,40).map((r)=>`<tr><td>${esc(r.fundraiserLabel)}</td><td><strong>${esc(r.description)}</strong></td><td>${esc(r.packageCompositionLabel||'—')}</td><td>${esc(r.brandScope||'Unclassified')}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${money(r.totalPremiumCost,2)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Category trends</h2><p>Component categories overlap, so these rows are comparative and must not be summed together. Bundle itself is structural, not a component category.</p></div>${table(['Category tag','Packages','Fundraisers','Pledges','Pledged','Avg pledge','Cost %','Net after premium'],s.categories.map((r)=>`<tr><td><strong>${esc(r.category)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Package composition trends</h2><p>Groups different exact premiums that use the same component recipe, such as DVD + CD + Book.</p></div>${table(['Package composition','Exact packages','Titles','Fundraisers','Pledges','Pledged','Avg pledge','Cost %','Net after premium'],(s.compositions||[]).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.distinctTitles)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Brand / source scope</h2><p>Separates WNMU/local, PBS/national, program/title, and unclassified premium relationships without forcing uncertain assignments.</p></div>${table(['Brand / source','Packages','Titles','Fundraisers','Pledges','Pledged','Premium cost','Cost %','Net after premium'],(s.brandScopes||[]).map((r)=>`<tr><td><strong>${esc(r.scope)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.distinctTitles)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Program / topic context</h2><p>Current Program Library premium text may be used as a historical proxy. It is labeled as such rather than treated as verified history.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Packages','Pledges','Premium-linked ,s.programs.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Packages','Pledges','Premium-linked ,s.topics.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div></div></section>
...
,'Premium cost','Cost / total ,s.fundraiserAnalysis.map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${money(r.totalPledged)}</td><td>${num(r.totalPledges)}</td><td>${pct(r.premiumTakeRate)}</td><td>${money(r.anyPremiumDollars)}</td><td>${money(r.totalPremiumCost)}</td><td>${pct(r.premiumCostPercentOfTotal)}</td><td>${money(r.averagePremiumPledge)}</td><td>${money(r.averageNoPremiumPledge)}</td></tr>`))}</section>
      ...
      <section><div class="section-heading"><h2>Program / topic context</h2><p>Current Program Library premium text may be used as a historical proxy. It is labeled as such rather than treated as verified history.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Packages','Pledges','Premium-linked $','Cost %'],s.programs.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Packages','Pledges','Premium-linked $','Cost %'],s.topics.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Combined program / topic context</h2><p>Existing broadcast results and premium-report dollars are kept as separate measures. This lets the report compare the fundraising program with its premium economics without pretending the two sources have identical attribution.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedPrograms.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.programTitle)}</strong><br><small>${esc(r.fundraiserLabel)}</small></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedTopics.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.topic)}</strong></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Data confidence</h2></div><p class="data-quality-note">Mapping rows: ${num(quality['Historically verified']||0)} historically verified · ${num(quality['Current-offer proxy']||0)} current-offer proxy · ${num(quality['Strongly inferred']||0)} strongly inferred · ${num(quality.Unmapped||0)} unmapped. Reported premium costs exclude expenses not present in the source report, such as shipping/handling.</p></section>
    </article>`;
  }
...
,'Net after premium','Avg premium pledge','Avg no-premium pledge'],s.fundraiserAnalysis.map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${money(r.totalPledged)}</td><td>${num(r.totalPledges)}</td><td>${pct(r.premiumTakeRate)}</td><td>${money(r.anyPremiumDollars)}</td><td>${money(r.totalPremiumCost)}</td><td>${pct(r.premiumCostPercentOfTotal)}</td><td>${money(r.averagePremiumPledge)}</td><td>${money(r.averageNoPremiumPledge)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Exact premium packages</h2><p>Each selectable premium remains its own accounting record. The normalized composition lets us compare similar package recipes across different titles.</p></div>${table(['Fundraiser','Premium / package','Package composition','Pledges','Pledged','Avg pledge','Cost','Cost %','Est. net'],s.packages.slice(0,40).map((r)=>`<tr><td>${esc(r.fundraiserLabel)}</td><td><strong>${esc(r.description)}</strong></td><td>${esc(r.packageCompositionLabel||'—')}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${money(r.totalPremiumCost,2)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Category trends</h2><p>Component categories overlap, so these rows are comparative and must not be summed together. Bundle itself is structural, not a component category.</p></div>${table(['Category tag','Packages','Fundraisers','Pledges','Pledged','Avg pledge','Cost %'],s.categories.map((r)=>`<tr><td><strong>${esc(r.category)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Package composition trends</h2><p>Groups different exact premiums that use the same component recipe, such as DVD + CD + Book.</p></div>${table(['Package composition','Exact packages','Titles','Fundraisers','Pledges','Pledged','Avg pledge','Cost %'],(s.compositions||[]).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.distinctPackages)}</td><td>${num(r.distinctTitles)}</td><td>${num(r.fundraiserCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.averagePledge)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</section>
      <section><div class="section-heading"><h2>Program / topic context</h2><p>Current Program Library premium text may be used as a historical proxy. It is labeled as such rather than treated as verified history.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Packages','Pledges','Premium-linked $','Cost %'],s.programs.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Packages','Pledges','Premium-linked $','Cost %'],s.topics.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div></div></section>
...
,'Cost %','Net after premium'],s.programs.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Packages','Pledges','Premium-linked $','Cost %'],s.topics.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Combined program / topic context</h2><p>Existing broadcast results and premium-report dollars are kept as separate measures. This lets the report compare the fundraising program with its premium economics without pretending the two sources have identical attribution.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedPrograms.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.programTitle)}</strong><br><small>${esc(r.fundraiserLabel)}</small></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedTopics.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.topic)}</strong></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Data confidence</h2></div><p class="data-quality-note">Mapping rows: ${num(quality['Historically verified']||0)} historically verified · ${num(quality['Current-offer proxy']||0)} current-offer proxy · ${num(quality['Strongly inferred']||0)} strongly inferred · ${num(quality.Unmapped||0)} unmapped. Reported premium costs exclude expenses not present in the source report, such as shipping/handling.</p></section>
    </article>`;
  }
...
,'Cost %','Net after premium'],s.topics.slice(0,20).map((r)=>`<tr><td><strong>${esc(r.label)}</strong></td><td>${num(r.exactPremiumCount)}</td><td>${num(r.pledgeCount)}</td><td>${money(r.pledgedDollars)}</td><td>${pct(r.costPercentOfPledged)}</td><td>${money(r.estimatedNetAfterPremium)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Combined program / topic context</h2><p>Existing broadcast results and premium-report dollars are kept as separate measures. This lets the report compare the fundraising program with its premium economics without pretending the two sources have identical attribution.</p></div><div class="premium-two-col"><div><h3>Programs</h3>${table(['Program','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedPrograms.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.programTitle)}</strong><br><small>${esc(r.fundraiserLabel)}</small></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div><div><h3>Topics</h3>${table(['Topic','Broadcast $','$ / pledge hr','Premium-linked $','Premium cost'],s.combinedTopics.filter((r)=>r.premiumPackages).slice(0,20).map((r)=>`<tr><td><strong>${esc(r.topic)}</strong></td><td>${money(r.broadcastDollars)}</td><td>${money(r.broadcastDollarsPerHour)}</td><td>${money(r.pledgedDollars)}</td><td>${money(r.totalPremiumCost)}</td></tr>`))}</div></div></section>
      <section><div class="section-heading"><h2>Data confidence</h2></div><p class="data-quality-note">Mapping rows: ${num(quality['Historically verified']||0)} historically verified · ${num(quality['Current-offer proxy']||0)} current-offer proxy · ${num(quality['Strongly inferred']||0)} strongly inferred · ${num(quality.Unmapped||0)} unmapped. Reported premium costs exclude expenses not present in the source report, such as shipping/handling.</p></section>
    </article>`;
  }
