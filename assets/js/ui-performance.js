(() => {
  ...
  function topicDayLabel(record) {
    const topic = record?.topicDisplay || record?.topic || 'Unassigned';
    const day = dayLabel(record);
    if (!topic || day === 'Unknown day') return 'Unknown topic/day';
    return `${topic} · ${day}`;
  }

  function topicDaypartDayLabel(record) {
    const topic = record?.topicDisplay || record?.topic || 'Unassigned';
    const part = daypartBaseLabel(record);
    const day = dayLabel(record);
    if (!topic || day === 'Unknown day' || !part || part === 'Unknown day-part') return 'Unknown topic/day-part/day';
    return `${topic} · ${part} · ${day}`;
  }
  ...
  function topicDaySortKey(record) {
    const dayIndex = DAY_ORDER.indexOf(dayLabel(record));
    return dayIndex < 0 ? Number.MAX_SAFE_INTEGER : dayIndex;
  }

  function topicDaypartDaySortKey(record) {
    const dayIndex = DAY_ORDER.indexOf(dayLabel(record));
    const partLabel = daypartBaseLabel(record);
    const partIndex = DAYPART_DEFS.findIndex((entry) => entry.label === partLabel);
    if (dayIndex < 0 || partIndex < 0) return Number.MAX_SAFE_INTEGER;
    return (dayIndex * 100) + partIndex;
  }
  ...
  function secondaryTopicMatches(record, filterValue) {
    const key = utils.normalizeLookupKey(filterValue);
    if (!key) return true;
    const full = utils.normalizeLookupKey(record.secondaryTopic || '');
  ...
  function buildPerformanceRecords(inputs) {
    ...
    function applyMetadata(record, row, identity, temporal) {
      ...
      const secondaryTopic = record.identityTrusted && programRow
        ? derive.topicSecondary(programRow)
        : utils.firstNonEmpty(row?.topic_secondary, row?.secondary_topic, row?.__resolved_topic_secondary, '');
      if (!record.secondaryTopic) record.secondaryTopic = utils.normalizeText(secondaryTopic);
    ...
    driveRows.forEach((sourceRow, index) => {
      ...
      if (money.trusted) {
        const hasAiringMoney = record.airingRows > 0 && record.moneyTrusted;
        // Airing rows are the source of truth for day/date/time analytics. Drive rollup rows
        // can describe the same program/fundraiser again; do not add their dollars on top
        // of already-imported airing dollars, or weekday/day-part comparisons get inflated.
        if (!hasAiringMoney) record.amount += Number.isFinite(money.amount) ? money.amount : 0;
        record.moneyTrusted = true;
        const moneySourceLabel = hasAiringMoney ? `${money.source}:metadata-only` : money.source;
        if (!record.moneySources.includes(moneySourceLabel)) record.moneySources.push(moneySourceLabel);
      }
      record.driveRows += 1;
      applyMetadata(record, row, identity, temporal);
    });
    ...
    const records = [...events.values()].map((record) => ({
      ...
      secondaryTopic: utils.normalizeText(record.secondaryTopic || ''),
  ...
  function criterionDisplayName(criterion = perf().criterion) {
    switch (criterion) {
      ...
      case 'topic_daypart_day': return 'Topic by day-part by day';
      case 'topic': return 'Main topic';
      case 'secondary_topic': return 'Secondary topic';
  ...
  function criterionLabel(record, criterion) {
    switch (criterion) {
      ...
      case 'topic_day': return topicDayLabel(record);
      case 'topic_daypart_day': return topicDaypartDayLabel(record);
      case 'topic': return record.topicDisplay || 'Unassigned';
      case 'secondary_topic': return record.secondaryTopic || 'Unassigned secondary topic';
  ...
  function criterionOrderKey(group, criterion) {
    ...
    if (criterion === 'topic_day') return group.topicDaySortKey ?? Number.MAX_SAFE_INTEGER;
    if (criterion === 'topic_daypart_day') return group.topicDaypartDaySortKey ?? Number.MAX_SAFE_INTEGER;
  ...
  function effectiveChartType() {
    if (perf().criterion === 'topic_time' || perf().criterion === 'topic_dayset' || perf().criterion === 'topic_day') return 'heatmap';
    ...
    if ((perf().chartType === 'split_line' || perf().chartType === 'heatmap') && !['daypart', 'topic_time', 'topic_dayset', 'topic_day'].includes(perf().criterion)) {
  ...
  function temporalEligibility(record, criterion) {
    ...
    if (criterion === 'topic_day') return record.hasDate && !record.estimatedOnly;
    if (criterion === 'topic_daypart_day') return record.hasDate && record.hasExplicitTime && !record.estimatedOnly;
  ...
  function integrityEligible(record, criterion) {
    ...
    const needsTrustedIdentity = criterion === 'program' || criterion === 'topic' || criterion === 'secondary_topic' || criterion === 'topic_time' || criterion === 'topic_dayset' || criterion === 'topic_day' || criterion === 'topic_daypart_day';
    const needsMainTopic = criterion === 'topic' || criterion === 'topic_time' || criterion === 'topic_dayset' || criterion === 'topic_day' || criterion === 'topic_daypart_day';
    const needsSecondaryTopic = criterion === 'secondary_topic';
    if (needsTrustedIdentity && !record?.identityTrusted) return false;
    if (needsMainTopic && !(record?.topicTokens || []).length) return false;
    if (needsSecondaryTopic && !utils.normalizeText(record?.secondaryTopic || '')) return false;
    return true;
  }
  ...
  function exclusionReasons(record, criterion) {
    ...
    if ((criterion === 'program' || criterion === 'topic' || criterion === 'secondary_topic' || criterion === 'topic_time' || criterion === 'topic_dayset' || criterion === 'topic_day' || criterion === 'topic_daypart_day') && !record?.identityTrusted) reasons.push('Weak program identity');
    if ((criterion === 'topic' || criterion === 'topic_time' || criterion === 'topic_dayset' || criterion === 'topic_day' || criterion === 'topic_daypart_day') && !(record?.topicTokens || []).length) reasons.push('Missing main topic');
    if (criterion === 'secondary_topic' && !utils.normalizeText(record?.secondaryTopic || '')) reasons.push('Missing secondary topic');
  ...
  function filterAndGroupRecords() {
    ...
    records.forEach((record) => {
      ...
      if (!groups.has(label)) {
        groups.set(label, {
          ...
          topicDaySortKey: null,
          topicDaypartDaySortKey: null,
          dayIndex: null,
          hourOfDay: null
        });
      }
      ...
      if (Number.isFinite(record.topicDaySortKey)) {
        group.topicDaySortKey = group.topicDaySortKey == null ? record.topicDaySortKey : Math.min(group.topicDaySortKey, record.topicDaySortKey);
      }
      if (Number.isFinite(record.topicDaypartDaySortKey)) {
        group.topicDaypartDaySortKey = group.topicDaypartDaySortKey == null ? record.topicDaypartDaySortKey : Math.min(group.topicDaypartDaySortKey, record.topicDaypartDaySortKey);
      }
    ...
    perf().analysisMeta = {
      ...
      excludedMissingSecondaryTopicCount: analyticsScopedRecords.filter((record) => !utils.normalizeText(record.secondaryTopic || '')).length,
  ...
  function buildTopicDayHeatmap(groups) {
    const orderedRows = [];
    const rowMap = new Map();
    const colOrder = DAY_ORDER;
    (groups || []).forEach((group) => {
      const bits = String(group.label || '').split(' · ');
      const day = bits.pop();
      const topic = bits.join(' · ') || group.label;
      if (!colOrder.includes(day)) return;
      if (!rowMap.has(topic)) rowMap.set(topic, { topic, cells: new Map() });
      rowMap.get(topic).cells.set(day, group);
    });
    orderedRows.push(...[...rowMap.values()].sort((a, b) => {
      const aBest = Math.max(...colOrder.map((col) => metricValue(a.cells.get(col) || emptyMetricCell())));
      const bBest = Math.max(...colOrder.map((col) => metricValue(b.cells.get(col) || emptyMetricCell())));
    ...
    if (!orderedRows.length) return '<div class="performance-chart-empty">Topic by day needs trustworthy dated rows with topic metadata in the active filter window.</div>';
    const maxMetric = Math.max(1, ...orderedRows.flatMap((row) => colOrder.map((col) => metricValue(row.cells.get(col) || emptyMetricCell()))));
    const header = colOrder.map((label) => `<div class="topic-time-header-cell">${utils.escapeHtml(label)}</div>`).join('');
    const body = orderedRows.map((row) => {
      const cells = colOrder.map((col) => {
    ...
    return `
      <div class="topic-time-heatmap-wrap topic-day-heatmap-wrap">
        <div class="topic-time-header"><div class="topic-time-corner">Topic / day</div>${header}</div>
        <div class="topic-time-grid">${body}</div>
      </div>
      <div class="performance-chart-note">This view splits each topic by exact broadcast day. It uses imported airing/event rows and does not add drive-rollup dollars on top of matching airing dollars.</div>
    `;
  }
  ...
  const SLOT_FIT_CACHE_VERSION = '0.21.30';
  ...
  const HOLIDAY_TOPIC_REGEX = /\b(holiday|holidays|christmas|xmas|yuletide|yule|noel|santa|reindeer|nutcracker|carol|carols|hanukkah|chanukah|kwanzaa|thanksgiving|new\s*year|new\s*year'?s|winterfest)\b/i;
  const MONTH_ALIASES = [
    ['January', /\b(january|jan\.?)(?=\s|\d|$)/i],
    ['February', /\b(february|feb\.?)(?=\s|\d|$)/i],
    ['March', /\b(march|mar\.?)(?=\s|\d|$)/i],
    ['April', /\b(april|apr\.?)(?=\s|\d|$)/i],
    ['May', /\bmay\b/i],
    ['June', /\b(june|jun\.?)(?=\s|\d|$)/i],
    ['July', /\b(july|jul\.?)(?=\s|\d|$)/i],
    ['August', /\b(august|aug\.?)(?=\s|\d|$)/i],
    ['September', /\b(september|sept\.?|sep\.?)(?=\s|\d|$)/i],
    ['October', /\b(october|oct\.?)(?=\s|\d|$)/i],
  ...
  function monthNameFromText(value = '') {
    const text = utils.normalizeText(value || '');
    if (!text) return '';
    const found = MONTH_ALIASES.find(([, pattern]) => pattern.test(text));
  ...
  function scheduleSeasonLabelForRecord(record = {}) {
    ...
    const candidateRanges = sourceRows.map((row) => ({
      start: utils.normalizeText(row?.drive_start_date || row?.fundraiser_start_date || row?.start_date || ''),
      end: utils.normalizeText(row?.drive_end_date || row?.fundraiser_end_date || row?.end_date || ''),
  ...
  function recordLooksHolidayRelated(record = {}) {
    const sourceText = (Array.isArray(record.sampleSourceRows) ? record.sampleSourceRows : [])
      .map((row) => [row?.program_title, row?.imported_program_title, row?.title, row?.matched_library_title, row?.premium_summary, row?.notes].map((item) => utils.normalizeText(item)).filter(Boolean).join(' '))
      .join(' ');
    const text = [record.title, record.importedTitle, record.matchedLibraryTitle, record.topicDisplay, record.premiums, sourceText]
      .map((item) => utils.normalizeText(item))
      .filter(Boolean)
      .join(' ');
    return HOLIDAY_TOPIC_REGEX.test(text);
  }

  function topicSeasonEligibleRecords(records = []) {
    return (Array.isArray(records) ? records : []).filter((record) => {
      if (!record || isNonSpecificRecord(record) || record.excludedForIntegrity) return false;
      if (!record.identityTrusted || !record.moneyTrusted || !record.hasDate || !recordFilterDate(record)) return false;
      if (!(record.topicTokens || []).length) return false;
      if (recordLooksHolidayRelated(record)) return false;
      return true;
    });
  }

  function addTopicSeasonStat(map, key, seed = {}) {
    if (!map.has(key)) {
      map.set(key, {
        key,
        topic: seed.topic || '',
  ...
  function topicSeasonLiftClass(lift = 0, lowSample = false) {
    if (lowSample) return 'low-sample';
    if (lift >= 0.25) return 'strong-positive';
    if (lift >= 0.10) return 'positive';
    if (lift <= -0.25) return 'strong-negative';
    if (lift <= -0.10) return 'negative';
    return 'neutral';
  }

  function formatLiftPct(lift = 0) {
    const pct = Math.round((Number(lift || 0) || 0) * 100);
    return `${pct >= 0 ? '+' : ''}${pct}%`;
  }
  ...
  function renderChart(groups) {
    ...
    if (perf().quickFilter === 'topic_season_lift') {
      els.performanceChart.innerHTML = buildTopicSeasonLiftHeatmap(perf().filteredRecords || []);
      return;
    }
    if (perf().quickFilter === 'topic_slot_winners') {
    ...
    if (perf().criterion === 'topic_day') {
      els.performanceChart.innerHTML = buildTopicDayHeatmap(groups);
      return;
    }
  ...
  function renderTable(groups) {
    ...
    setPerformanceTableHeaders(perf().criterion === 'topic_daypart_day'
      ? ['Topic · day-part · day', 'Airings', 'Total dollars', metricLabel(), 'Confidence', 'Titles']
      : [criterionDisplayName(), 'Airings', 'Total dollars', metricLabel(), 'Confidence', 'Titles']);
    if (!groups.length) {
  ...
  function slotFitAccuracyBuckets(rows = []) {
    const buckets = [
      { key: 'close', label: 'Close', detail: 'Within 25%', count: 0 },
      { key: 'under', label: 'Under-projected', detail: 'Actual was higher', count: 0 },
      { key: 'over', label: 'Over-projected', detail: 'Actual was lower', count: 0 }
    ];
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      const projected = Number(row.projected || 0);
      const actual = Number(row.actual || 0);
      const absError = Math.abs(actual - projected);
      const ratio = projected > 0 ? absError / projected : (actual === 0 ? 0 : 1);
      if (ratio <= 0.25) buckets[0].count += 1;
      else if (actual > projected) buckets[1].count += 1;
      else buckets[2].count += 1;
    });
    return buckets;
  }

  function buildSlotFitAccuracyBars(rows = []) {
    const buckets = slotFitAccuracyBuckets(rows);
    const total = buckets.reduce((sum, bucket) => sum + bucket.count, 0) || 1;
    const max = Math.max(1, ...buckets.map((bucket) => bucket.count));
    return `
      <div class="slot-fit-accuracy-bars" aria-label="Slot Fit projection accuracy buckets">
        ${buckets.map((bucket) => {
          const pct = Math.round((bucket.count / total) * 100);
          const width = Math.max(3, Math.round((bucket.count / max) * 100));
          return `
            <div class="slot-fit-accuracy-bar-row ${utils.escapeHtml(bucket.key)}">
              <div class="slot-fit-accuracy-bar-label"><strong>${utils.escapeHtml(bucket.label)}</strong><span>${utils.escapeHtml(bucket.detail)}</span></div>
              <div class="slot-fit-accuracy-bar-track"><div class="slot-fit-accuracy-bar-fill" style="width:${utils.escapeHtml(String(width))}%"></div></div>
              <div class="slot-fit-accuracy-bar-value">${utils.escapeHtml(utils.formatCount(bucket.count))} · ${utils.escapeHtml(`${pct}%`)}</div>
            </div>
          `;
        }).join('')}
      </div>
  ...
  function renderSlotFitAccuracy() {
    ...
    els.performanceSlotFitAccuracy.innerHTML = `
      ...
      ${buildSlotFitAccuracyBars(rows)}
      <div class="table-wrap performance-table-wrap slot-fit-table-wrap">
        <table class="programs-table performance-table slot-fit-accuracy-table">
          <thead><tr><th>Recent fundraiser</th><th>Title</th><th>Projected</th><th>Actual</th><th>Difference</th></tr></thead>
          <tbody>
            ${recent.map((row) => `<tr><td>${utils.escapeHtml(row.scheduleTitle)}</td><td>${utils.escapeHtml(row.programTitle)}</td><td>${utils.escapeHtml(utils.formatMoney(row.projected))}</td><td>${utils.escapeHtml(utils.formatMoney(row.actual))}</td><td class="${row.error >= 0 ? 'positive-diff' : 'negative-diff'}">${utils.escapeHtml(`${row.error >= 0 ? '+' : '-'}${utils.formatMoney(Math.abs(row.error))}`)}</td></tr>`).join('')}
          </tbody>
        </table>
      </div>
    `;
  ...
  function collectSecondaryTopicOptions() {
    const labels = new Map();
    const mainTopicFilter = perf().topicFilter || '';
    (state.rawRows || []).forEach((row) => {
      if (mainTopicFilter && !topicMatches({ topicDisplay: derive.topicPrimary(row), topic: derive.topicPrimary(row), topicTokens: [derive.topicPrimary(row)].filter(Boolean) }, mainTopicFilter)) return;
      const token = utils.normalizeText(derive.topicSecondary(row));
      const key = utils.normalizeLookupKey(token);
      if (key && !labels.has(key)) labels.set(key, token);
    });
    (perf().records || []).forEach((record) => {
      if (mainTopicFilter && !topicMatches(record, mainTopicFilter)) return;
      const token = utils.normalizeText(record.secondaryTopic || '');
      const key = utils.normalizeLookupKey(token);
      if (key && !labels.has(key)) labels.set(key, token);
    });
    return [...labels.values()].sort((a, b) => utils.compareText(a, b));
  ...
  function quickFilterLabel(name = perf().quickFilter) {
    const labels = {
      ...
      topic_by_daypart_day: 'Topic by day-part by day',
      topic_season_lift: 'Topic seasonal lift',
  ...
  function quickFilterExplanation(name = perf().quickFilter) {
    ...
    switch (name) {
      ...
      case 'topic_by_day':
        return 'Splits each main topic by exact day of week, using imported airing/event records so you can see whether a topic really works better on specific days.';
  ...
  function quickFilterLearningSummary(name) {
    switch (name) {
      ...
      case 'topic_by_day':
        return 'Use this when the exact day matters and the broad weekday/weekend split is too blunt.';
      case 'topic_by_daypart_day':
  ...
  function populateControls() {
    ...
    const secondaryTopicOptions = collectSecondaryTopicOptions();
    if (perf().secondaryTopicFilter && !secondaryTopicOptions.includes(perf().secondaryTopicFilter)) perf().secondaryTopicFilter = '';
    renderSelectOptions(els.performanceSecondaryTopicSelect, secondaryTopicOptions, perf().secondaryTopicFilter, 'All secondary topics');
    ...
    if (els.performanceSecondaryTopicSelect) els.performanceSecondaryTopicSelect.value = perf().secondaryTopicFilter;
    updateQuickFilterUi();
  }
  ...
  function renderAll() {
    ...
    if (els.performanceChartTitle) {
      const focus = (['topic_time', 'topic_dayset', 'topic_day', 'topic_daypart_day'].includes(perf().criterion) && perf().topicFilter)
        ? ` · ${perf().topicFilter}`
      ...
      els.performanceChartTitle.textContent = perf().quickFilter === 'topic_slot_winners'
        ...
        : perf().quickFilter === 'topic_season_lift'
          ? 'Topic seasonal lift'
          : `${metricLabel()} by ${criterionDisplayName()}${focus}`;
    }
    if (els.performanceChartPill) {
      const vizLabel = (['topic_time', 'topic_dayset', 'topic_day'].includes(perf().criterion) || ['topic_slot_winners', 'topic_season_lift'].includes(perf().quickFilter)) ? 'Heatmap' : chartTypeLabel(effectiveChartType());
      const chartSubject = perf().quickFilter === 'topic_slot_winners' ? 'Topic winners' : perf().quickFilter === 'topic_season_lift' ? 'Seasonal lift' : criterionDisplayName();
      els.performanceChartPill.textContent = perf().ready ? `${vizLabel} · ${chartSubject}` : 'Awaiting data';
    }
  ...
  const QUICK_FILTERS = {
    top_earners: { criterion: 'program', metric: 'total_dollars', chartType: 'bar', topN: 12, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    best_average: { criterion: 'program', metric: 'avg_dollars', chartType: 'bar', topN: 12, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    prime_time: { criterion: 'program', metric: 'avg_dollars', chartType: 'bar', topN: 12, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: 'Prime time', weekpartScope: '' },
    live_break_impact: { criterion: 'live_breaks', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    repeat_fatigue: { criterion: 'program', metric: 'avg_dollars', chartType: 'bar', topN: 12, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    ...
    topic_by_daypart_day: { criterion: 'topic_daypart_day', metric: 'avg_dollars', chartType: 'bar', topN: 999, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    topic_season_lift: { criterion: 'topic', metric: 'avg_dollars', chartType: 'heatmap', topN: 999, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    recent_momentum: { criterion: 'program', metric: 'avg_dollars', chartType: 'bar', topN: 10, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    weekend_weekday: { criterion: 'weekpart', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    day_of_week: { criterion: 'day', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    daypart: { criterion: 'daypart', metric: 'avg_dollars', chartType: 'split_line', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    weekday_dayparts: { criterion: 'daypart', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: 'mon_fri', daypartScope: '', weekpartScope: '' },
    saturday_dayparts: { criterion: 'daypart', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: 'saturday', daypartScope: '', weekpartScope: '' },
    sunday_dayparts: { criterion: 'daypart', metric: 'avg_dollars', chartType: 'bar', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: 'sunday', daypartScope: '', weekpartScope: '' },
    daypart_heatmap: { criterion: 'daypart', metric: 'avg_dollars', chartType: 'heatmap', topN: 8, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: '', weekpartScope: '' },
    best_daytime: { criterion: 'program', metric: 'avg_dollars', chartType: 'bar', topN: 10, monthFilter: '', topicFilter: '', secondaryTopicFilter: '', labelFilter: '', programFilter: '', daySetFilter: '', daypartScope: 'Daytime', weekpartScope: '' }
  };
