...
test('v0.22.131 puts visual summaries before detailed report tables', () => {
...
test('v0.22.131 adds trend, lifecycle, topic, start-time, and donor-context views', () => {
  for (const marker of [
...
test('report assets and application version are synchronized at v0.22.131', () => {
  assert.equal(version.appVersion, '0.22.131');
  assert.ok(html.includes('one-sheet-reports.css?v=0.22.131'));
  assert.ok(html.includes('one-sheet-analysis.js?v=0.22.131'));
  assert.ok(html.includes('one-sheet-reports.js?v=0.22.131'));
});
