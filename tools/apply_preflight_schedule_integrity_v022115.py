...
new_imported_rows = r'''  function importedRowsForSchedule(schedule = {}, allRows = []) {
...
'''
analysis = replace_region(
    analysis,
    '  function importedRowsForSchedule(schedule = {}, allRows = []) {',
    '  function importedUseKey(row = {}) {',
    new_imported_rows,
    'replace season-wide imported-row fallback'
)

integrity_helpers = r'''  function preflightImportedCoverageFindings(schedules = [], airings = []) {
...
'''
health_marker = '  function dataHealthReport(schedules = [], analyses = [], airings = [], library = []) {'
if analysis.count(health_marker) != 1:
    raise SystemExit('dataHealthReport insertion marker not unique')
analysis = analysis.replace(health_marker, integrity_helpers + health_marker, 1)

health_pos = analysis.find(health_marker)
reconciliation_pos = analysis.find('    const reconciliation = [];', health_pos)
if reconciliation_pos < 0:
    raise SystemExit('reconciliation insertion marker not found')
coverage_block = r'''    const scheduleCoverage = preflightImportedCoverageFindings(schedules, airings);
...
'''
analysis = analysis[:reconciliation_pos] + coverage_block + analysis[reconciliation_pos:]
...
analysis = replace_once(
    ...
    'make Preflight imported-row metric dataset-wide'
)
analysis = replace_once(
    analysis,
    '    canonicalizeImportedAirings,\n    libraryRuntimeMinutes,',
    '    canonicalizeImportedAirings,\n    importedDateKey,\n    importedStartMinutes,\n    importedTitle,\n    libraryRuntimeMinutes,',
    'export imported airing helpers for repair preview'
)
...
preflight_helpers = r'''  function preflightResolveLibraryRow(row = {}) {
...
'''
reports_marker = '  function ensurePreflightProgramEditor() {'
if reports.count(reports_marker) != 1:
    raise SystemExit('Preflight helper insertion marker not unique')
reports = reports.replace(reports_marker, preflight_helpers + reports_marker, 1)

bind_start = reports.find('  function bindPreflightProgramEditor() {')
click_start = reports.find("    output.addEventListener('click', (event) => {", bind_start)
key_start = reports.find("    document.addEventListener('keydown', (event) => {", click_start)
if min(bind_start, click_start, key_start) < 0:
    raise SystemExit('Preflight delegated click region not found')
new_click = r'''    output.addEventListener('click', (event) => {
...
'''
reports = reports[:click_start] + new_click + reports[key_start:]

detail_function = reports.find('  function preflightCheckMarkup(check) {')
detail_start = reports.find('      const mismatchTypes = Array.isArray(item.mismatchTypes) ? item.mismatchTypes.filter(Boolean) : [];', detail_function)
detail_end = reports.find('    const detailMarkup = details.length', detail_start)
if min(detail_function, detail_start, detail_end) < 0:
    raise SystemExit('Preflight detail rendering region not found')
new_detail_tail = r'''      const mismatchTypes = Array.isArray(item.mismatchTypes) ? item.mismatchTypes.filter(Boolean) : [];
...
'''
reports = reports[:detail_start] + new_detail_tail + reports[detail_end:]

reports = replace_once(
    reports,
    'PASS means no blocking defects were found in Broadcast reconciliation, program duration coverage, imported program attribution, or duplicate fundraiser ranges.',
    'PASS means no blocking defects were found in fundraiser schedule coverage, Broadcast reconciliation, program duration coverage, imported program attribution, or duplicate fundraiser ranges.',
    'update Preflight footer scope'
)
reports_path.write_text(reports, encoding='utf-8')

html_path = Path('reports.html')
html = html_path.read_text(encoding='utf-8').replace('0.22.114', '0.22.115')
html_path.write_text(html, encoding='utf-8')

version_path = Path('version.json')
version = json.loads(version_path.read_text(encoding='utf-8'))
if version.get('appVersion') != '0.22.114':
    raise SystemExit(f"Expected v0.22.114 before bump, found {version.get('appVersion')}")
version['appVersion'] = '0.22.115'
version['releasedAt'] = '2026-09-01'
version_path.write_text(json.dumps(version, separators=(',', ':')) + '\n', encoding='utf-8')

test_path = Path('tests/data-health-preflight.test.mjs')
